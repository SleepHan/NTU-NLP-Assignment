...
def process_batch_bert(batch, text_vocab, device, debug=False):
    text, text_lengths = batch.text
    labels = batch.label
...
def train_model_with_history_bert(model, train_iterator, val_iterator, optimizer, criterion, 
    ...
    for epoch in range(n_epochs):
        ...
        for batch in train_iterator:
            text, text_lengths, labels, _ = process_batch_bert(batch, text_vocab, device, debug=False)
            optimizer.zero_grad()
...
def evaluate_model_bert(model, iterator, criterion, device, model_name, num_classes, text_vocab=None):
